...
import os # perform os specific tasks
import json # work with json data
import datetime # work with dates

# Make database connection
db = f"{os.getcwd()}/library.db"
...
def initialise_db():
    # Create books table
    cur.execute("""CREATE TABLE IF NOT EXISTS books(id INTEGER PRIMARY KEY AUTOINCREMENT, book_id TEXT, title TEXT, author TEXT, available BOOLEAN, borrower TEXT, borrowed_date TEXT)""")
    conn.commit()

    # Check if books table contains any books
    cur.execute("SELECT count(*) FROM books")
    result = cur.fetchone()
    book_count = result[0]

    # If no books are found preload books
    if book_count == 0:
        print("Pre-loading books ...")
        __preload_books()
        print("Pre-loading books complete!")
